...
import { Camera, Edit, Bookmark } from "lucide-react"
  ...
  const handleGenerate = async () => {
    ...
    try {
      for (let i = 0; i < variations[0]; i++) {
        ...
        if (i === 0 && !currentProject) {
          createProject(designObject)
        } else if (currentProject) {
          addDesignsToProject(currentProject.id, [designObject])
        }

        setDesigns((prev) => [newDesigns[newDesigns.length - 1], ...prev])
      }
      ...
      setToastMessage(`🎉 ${variations[0]} design${variations[0] > 1 ? "s" : ""} generated successfully!`)
      setShowToast(true)
    } catch (error) {
                ...
                <div className="p-6 bg-slate-900/50 backdrop-blur-xl border border-white/10 rounded-2xl">
                  ...
                  <div className="space-y-3">
                    ...
                    >
                      ...
                      Create Mockups ({selectedDesignIds.length} {selectedDesignIds.length === 1 ? "design" : "designs"}
                      )
                    </button>
                    ...
                    <button
                      onClick={() => {
                        toast({
                          ...
                          description: "Edit your selected designs",
                        })
                        router.push("/edit-design")
                      }}
